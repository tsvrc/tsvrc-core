...
{
    // PlayerPositionOverlay IS a PlayerTracker (via Process), so
    ...
    // Tests/PlayMode/UI/PlayerPositionOverlay/ instead.
    public class TsPlayerPositionOverlayTests : PlayerTrackerTestBase
    {
        ...
        private static void SetupValid(PlayerPositionOverlay overlay, int width = 10, int height = 10, float ppu = 1f)
        {
            overlay.Setup(width, height, Vector3.zero, ppu, ppu);
        }

        private static bool IsSetUp(PlayerPositionOverlay overlay) =>
        ...
        private static Texture2D GetTexture(PlayerPositionOverlay overlay) =>
            PrivateFieldAccess.GetField<Texture2D>(overlay, "_overlayTexture");

        private static Color32[] GetPixelBuffer(PlayerPositionOverlay overlay) =>
        ...
        public void Setup_NullOverlayImage_LogsErrorAndDoesNotMarkSetup()
        {
            ...
            LogAssert.Expect(LogType.Error, "[TsVRC] [PlayerPositionOverlay] OverlayImage is not assigned.");
        ...
        public void Setup_NonPositiveWidth_LogsErrorAndDoesNotMarkSetup()
        {
            ...
            LogAssert.Expect(LogType.Error, "[TsVRC] [PlayerPositionOverlay] Texture dimensions must be positive.");
        ...
        public void Setup_NonPositivePixelsPerUnitX_LogsErrorAndDoesNotMarkSetup()
        {
            ...
            LogAssert.Expect(LogType.Error, "[TsVRC] [PlayerPositionOverlay] PixelsPerUnit values must be positive.");
        ...
        public void StartOverlay_BeforeSetup_LogsErrorAndDoesNotStartTracking()
        {
            ...
            LogAssert.Expect(LogType.Error, "[TsVRC] [PlayerPositionOverlay] Setup() must be called before StartOverlay().");

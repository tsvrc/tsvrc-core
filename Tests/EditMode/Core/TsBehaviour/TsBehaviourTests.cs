...
{
    // TsBehaviour never touches Networking/VRCPlayerApi, so nothing here needs
    // ClientSim or Play Mode.
    public class TsBehaviourTests
    {
        ...
        {
            var parentA = CreateBehaviour<TsBehaviourTestSubclass>("ParentA");
            var parentB = CreateBehaviour<TsBehaviourTestSubclass>("ParentB");
            var rootA = CreateBehaviour<TestTsRoot>();
            var rootB = CreateBehaviour<TestTsRoot>();
            parentA.TsConstruct(rootA);
            parentB.TsConstruct(rootB);
            var child = CreateBehaviour<TsBehaviourTestSubclass>("Child");
        ...
        {
            var parent = CreateBehaviour<TsBehaviourTestSubclass>("Parent");
            var root = CreateBehaviour<TestTsRoot>();
        ...
        {
            var child = CreateBehaviour<TsBehaviourTestSubclass>();

            Assert.Throws<NullReferenceException>(() => child.TsConstruct((TsBehaviour)null));
        }
        ...
        public void TsConstruct_Parent_AlreadyConstructed_SecondCallWithNullParent_IsANoOpAndDoesNotThrow()
        {
            ...
            Assert.DoesNotThrow(() => child.TsConstruct((TsBehaviour)null));
            Assert.AreEqual(1, child.TsStartCallCount);
            Assert.AreSame(root, child.GetTs());
        }
        ...
        {
            var root = CreateBehaviour<TestTsRoot>();
            var a = CreateBehaviour<TsBehaviourTestSubclass>("A");
            a.TsConstruct(root);
            var b = CreateBehaviour<TsBehaviourTestSubclass>("B");
            b.TsConstruct(a);
            var c = CreateBehaviour<TsBehaviourTestSubclass>("C");
        ...
        {
            var publisher = CreateBehaviour<TsBehaviourTestSubclass>();
        ...
        {
            var behaviour = CreateBehaviour<TsBehaviourNoDestroySubclass>();
            GameObject go = behaviour.gameObject;
        ...
        {
            var behaviour = CreateBehaviour<TsBehaviour>();

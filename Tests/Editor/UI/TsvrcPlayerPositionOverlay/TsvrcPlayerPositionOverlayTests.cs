...
{
    // TsvrcPlayerPositionOverlay IS a PlayerTracker (via TsvrcProcess), so
    ...
    // Tests/PlayMode/UI/TsvrcPlayerPositionOverlay/ instead.
    public class TsvrcPlayerPositionOverlayTests : PlayerTrackerTestBase
    {
        private TsvrcPlayerPositionOverlay CreateOverlay()
        {
            var overlay = CreateProcess<TsvrcPlayerPositionOverlay>();
            overlay.OverlayImage = overlay.gameObject.AddComponent<RawImage>();
            return overlay;
        }

        private static void SetupValid(TsvrcPlayerPositionOverlay overlay, int width = 10, int height = 10, float ppu = 1f)
        {
            overlay.Setup(width, height, Vector3.zero, ppu, ppu);
        }

        private static bool IsSetUp(TsvrcPlayerPositionOverlay overlay) =>
            PrivateFieldAccess.GetField<bool>(overlay, "_isSetup");

        private static bool IsOverlayUpdating(TsvrcPlayerPositionOverlay overlay) =>
            PrivateFieldAccess.GetField<bool>(overlay, "_isOverlayUpdating");

        private static Texture2D GetTexture(TsvrcPlayerPositionOverlay overlay) =>
            PrivateFieldAccess.GetField<Texture2D>(overlay, "_overlayTexture");

        private static Color32[] GetPixelBuffer(TsvrcPlayerPositionOverlay overlay) =>
        ...
        private static void SeedRunningAndUpdating(TsvrcPlayerPositionOverlay overlay)
        {
            SeedAsOwner(overlay);
        ...
        public void Setup_NullOverlayImage_LogsErrorAndDoesNotMarkSetup()
        {
            ...
            LogAssert.Expect(LogType.Error, "[TsvrcPlayerPositionOverlay] OverlayImage is not assigned.");
        ...
        public void Setup_NonPositiveWidth_LogsErrorAndDoesNotMarkSetup()
        {
            ...
            LogAssert.Expect(LogType.Error, "[TsvrcPlayerPositionOverlay] Texture dimensions must be positive.");
        ...
        public void Setup_NonPositivePixelsPerUnitX_LogsErrorAndDoesNotMarkSetup()
        {
            ...
            LogAssert.Expect(LogType.Error, "[TsvrcPlayerPositionOverlay] PixelsPerUnit values must be positive.");
        ...
        public void StartOverlay_BeforeSetup_LogsErrorAndDoesNotStartTracking()
        {
            ...
            LogAssert.Expect(LogType.Error, "[TsvrcPlayerPositionOverlay] Setup() must be called before StartOverlay().");
        ...
        public void StopOverlay_Running_FiresOnOverlayUpdatedEventEvenWithNothingDrawn()
        {
            ...
            overlay.TsSubscribe(listener, TsvrcPlayerPositionOverlay.OnOverlayUpdatedEvent, nameof(TsvrcListenerDouble.CallbackA));
        ...
        private static int ParsePlayerIntId(TsvrcPlayerPositionOverlay overlay, string id) =>

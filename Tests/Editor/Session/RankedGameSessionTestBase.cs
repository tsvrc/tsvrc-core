...
namespace Tsvrc.Tests.Editor
{
    ...
    // ReadyCheckProcess, a TsvrcTimer) rather than inheriting from TsvrcProcess itself,
    // so this base extends TsvrcProcessTestBase directly for CreateComponent<T>/
    ...
    // hierarchy TsvrcGenerator's InstanceModule would wire at runtime through
    // [WirePool], reproduced here without depending on the generator.
    public abstract class RankedGameSessionTestBase : TsvrcProcessTestBase
    {
        protected class Harness
        {
            ...
            public TsvrcTimerTestSubclass Timer;
        }

        // Builds a fully-wired session with a TsvrcRoot double that has no overridden
        ...
        // itself need a real, non-null TsvrcInstance; see CreateWiredSession(TsvrcRoot)).
        protected Harness CreateWiredSession()
        {
            var h = CreateWiredSession(new TestTsvrcRoot());
            SetMasterOnly(h.Session, false);
            return h;
        }

        protected Harness CreateWiredSession(TsvrcRoot root)
        {
            var h = new Harness
            {
                ...
                Timer = CreateProcess<TsvrcTimerTestSubclass>(),
            };

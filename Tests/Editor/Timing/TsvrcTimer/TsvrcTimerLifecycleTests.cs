...
namespace Tsvrc.Tests.Editor
{
    ...
    public class TsvrcTimerLifecycleTests : TsvrcTimerTestBase
    {
        [Test]
        public void StartTimer_Parameterless_OpenEndedNoProcessUpdate()
        {
            var timer = CreateProcess<TsvrcTimerTestSubclass>();
            SeedAsOwner(timer);
        ...
        {
            // Unlike TsvrcProcess.StartProcess's own "already running" guard (which logs
            // a warning), TsvrcTimer.StartTimer()'s own outer IsProcessRunning() guard
        ...
        public void LocalUpdateInterval_ConstantIsPinnedAtQuarterSecond()
        {
            ...
            float interval = PrivateFieldAccess.GetField<float>(typeof(TsvrcTimer), "_localUpdateInterval");
        ...
        public void ReentrantStopFromOnTimerStarted_UnwindsCleanlyAndSkipsTheBaseClassPostStartTickSchedule()
        {
            ...
            // TsvrcProcess.StartProcess's own callback) calls TsvrcTimer.OnTimerStarted()
        ...
        public void EventOrdering_Start_HookFiresBeforeEvent()
        {
            var timer = CreateProcess<TsvrcTimerTestSubclass>();
            SeedAsOwner(timer);
            timer.TsSubscribe(timer, TsvrcTimer.OnTimerStartedEvent, nameof(timer._OnTimerStartedEventReceived));
        ...
        public void EventOrdering_Stop_HookFiresBeforeEvent()
        {
            ...
            timer.StartTimer();
            timer.CallLog.Clear();
            timer.TsSubscribe(timer, TsvrcTimer.OnTimerStoppedEvent, nameof(timer._OnTimerStoppedEventReceived));
        ...
        public void FullLifecycle_StartPauseResumeStopRestartComplete_EndToEndDoesNotThrowAndEndsClean()
        {
            ...
            // in isolation elsewhere in this file and in TsvrcTimerPauseResumeTests.cs.
            var timer = CreateProcess<TsvrcTimerTestSubclass>();
            SeedAsOwner(timer);
            ...
            Assert.DoesNotThrow(() =>
            {
                ...
                // TsvrcProcess.InternalCleanup behavior the elapsed-math tests in
                // TsvrcTimerElapsedAndRemainingTests.cs rely on), so a genuinely fresh

...
namespace Tsvrc.Tests.Editor
{
    ...
    public class TsvrcTimerDeserializationTests : TsvrcTimerTestBase
    {
        private static void SeedObservedState(TsvrcTimerTestSubclass timer, bool hasObserved, bool wasRunning, bool wasPaused, int lastObservedRunId)
        {
        ...
        public void OnDeserialization_EventReceived_FiresAfterHookForEveryTransition()
        {
            var timer = CreateProcess<TsvrcTimerTestSubclass>();
            SeedAsOwner(timer);
            timer.TsSubscribe(timer, Tsvrc.Timing.TsvrcTimer.OnTimerDeserializationEvent, nameof(timer._OnTimerDeserializationEventReceived));
        ...
        // Needs two genuinely independent TsvrcTimerTestSubclass instances (owner +
        ...
        // a remote as a single simulated packet via TsvrcTimerTestBase.CopySyncedFieldsTo.

        [Test]
        public void OnDeserialization_CoalescedStopThenRestartInOnePacket_FiresStartedForNewRunNotOldRunEnd()
        {
            var owner = CreateProcess<TsvrcTimerTestSubclass>();
            var remote = CreateProcess<TsvrcTimerTestSubclass>();
            SeedAsOwner(owner);
            SeedAsOwner(remote);

...
{
    // Regression guards for two parts of TsvrcTimer's public contract that no
    ...
    // TsvrcTimerPauseResumeTests.cs - this is only about the attribute metadata itself
    // still being present and unchanged).
    public class TsvrcTimerContractRegressionTests : TsvrcTimerTestBase
    {
        [Test]
        public void EventConstants_LiteralStringValues_ArePinned()
        {
            Assert.AreEqual("OnTimerStarted", TsvrcTimer.OnTimerStartedEvent);
            Assert.AreEqual("OnTimerStopped", TsvrcTimer.OnTimerStoppedEvent);
            Assert.AreEqual("OnTimerCompleted", TsvrcTimer.OnTimerCompletedEvent);
            Assert.AreEqual("OnTimerPaused", TsvrcTimer.OnTimerPausedEvent);
            Assert.AreEqual("OnTimerResumed", TsvrcTimer.OnTimerResumedEvent);
            Assert.AreEqual("OnTimerUpdated", TsvrcTimer.OnTimerUpdatedEvent);
            Assert.AreEqual("OnTimerDeserialization", TsvrcTimer.OnTimerDeserializationEvent);
        }
        ...
        // assembly isn't referenced by the Tsvrc.Tests.Editor asmdef (only by
        ...
        {
            MethodInfo method = typeof(TsvrcTimer).GetMethod(methodName);
            Assert.IsNotNull(method, methodName + " not found on TsvrcTimer. Signature changed?");
        ...
        {
            AssertHasNetworkCallableRateLimit(nameof(TsvrcTimer.RequestPauseTimer), 2);
        }

        [Test]
        public void RequestResumeTimer_NetworkCallableAttribute_RateLimitIsPinnedAtTwoPerSecond()
        {
            AssertHasNetworkCallableRateLimit(nameof(TsvrcTimer.RequestResumeTimer), 2);
        }

        // Mirrors TsvrcInstanceTests.TsStart_IsNotOverriddenByTsvrcInstance's own
        ...
        public void DoesNotOverride_OnOwnerAbandonedProcess()
        {
            ...
            Assert.AreEqual(typeof(Tsvrc.Core.TsvrcProcess), method.DeclaringType,
        ...
        // overloaded (UdonSharpBehaviour's own base declarations plus TsvrcProcess's
        ...
        // directly, since that type isn't resolvable from the Tsvrc.Tests.Editor
        // asmdef at all (established convention in this suite - see
        // TsvrcProcessTestBase's own header comment: real VRCPlayerApi usage is
        // Play-Mode-only).
        private static void AssertDoesNotOverride(string methodName)
        {
            MethodInfo match = null;
            foreach (MethodInfo candidate in typeof(TsvrcTimer).GetMethods(BindingFlags.Public | BindingFlags.Instance))
            {
            ...
            Assert.IsNotNull(match, methodName + "(1 param) not found on TsvrcTimer. Signature changed?");
            Assert.AreEqual(typeof(Tsvrc.Core.TsvrcProcess), match.DeclaringType);
        }

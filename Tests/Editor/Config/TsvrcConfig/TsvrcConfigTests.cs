...
{
    // TsvrcConfig is a pure data container - four public array fields, zero methods.
    ...
    public class TsvrcConfigTests
    {
        ...
        private TsvrcConfig CreateConfig()
        {
            var go = new GameObject(nameof(TsvrcConfig));
            _spawned.Add(go);
            return go.AddComponent<TsvrcConfig>();
        }
        ...
        public void AddComponent_FromThisAssembly_Succeeds()
        {
            ...
            // runtime assembly. TsvrcConfig deliberately isn't under one.
            TsvrcConfig config = CreateConfig();

            Assert.IsNotNull(config);
        }

        [Test]
        public void TsvrcConfig_IsPlainMonoBehaviour_NotUdonSharpBehaviour()
        {
            Assert.IsTrue(typeof(MonoBehaviour).IsAssignableFrom(typeof(TsvrcConfig)));
            Assert.IsFalse(typeof(UdonSharpBehaviour).IsAssignableFrom(typeof(TsvrcConfig)),
                "TsvrcConfig must never become an UdonSharpBehaviour - it's read only by editor " +
        ...
        public void SerializedObject_FindsAllFourFieldsByName()
        {
            ...
            Assert.IsNotNull(so.FindProperty(nameof(TsvrcConfig.Singletons)),
                "Singletons must be a genuinely serialized field for the generator/Inspector to read it.");
            Assert.IsNotNull(so.FindProperty(nameof(TsvrcConfig.PooledObjects)));
            Assert.IsNotNull(so.FindProperty(nameof(TsvrcConfig.Constructs)));
            Assert.IsNotNull(so.FindProperty(nameof(TsvrcConfig.Factories)));
        }
        ...
        public void Singletons_TooltipText_NamesTheRealGeneratedClass_NotAStaleApiShape()
        {
            ...
            // as fields on the TsvrcGenerated partial class, with no nested wrapper
            // type - the tooltip must describe that shape.
            FieldInfo field = typeof(TsvrcConfig).GetField(nameof(TsvrcConfig.Singletons));

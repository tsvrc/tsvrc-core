...
namespace Tsvrc.Tests.Editor
{
    ...
    internal static class CompiledRootFixture
    {
        ...
        private const string ScriptPath = "Assets/TsvrcGenerated/" + ScaffoldModule.CompiledClassName + ".cs";
        private const string AssetPath = "Assets/TsvrcGenerated/" + ScaffoldModule.CompiledClassName + ".asset";

        internal static Component AddTo(TempSceneScope scope, string name = "TestRoot_TsvrcGenerated")
        {
            var compiledType = ScaffoldModule.FindCompiledType();
            if (compiledType == null)
            {
                Assert.Ignore("Compiled TsvrcGenerated type not found in the AppDomain - " +
            ...
            Assert.IsNotNull(component, "Failed to add compiled TsvrcGenerated component to the test fixture GameObject.");
            return component;
        }
    }
}

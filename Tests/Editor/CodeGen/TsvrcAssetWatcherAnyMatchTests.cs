...
{
    // TsvrcAssetWatcher.AnyMatch is `private static` — reflection is the only way in.
    public class TsvrcAssetWatcherAnyMatchTests
    {
        private static bool AnyMatch(string[] paths, HashSet<string> watched)
        {
            MethodInfo method = typeof(TsvrcAssetWatcher).GetMethod("AnyMatch", BindingFlags.NonPublic | BindingFlags.Static);
            Assert.IsNotNull(method, "TsvrcAssetWatcher.AnyMatch method changed or was removed.");
        ...
        {
            // TsvrcGenerator builds WatchedPaths with StringComparer.OrdinalIgnoreCase, and
            ...
            // behavior is actually in effect end-to-end (TsvrcGenerator always supplies the

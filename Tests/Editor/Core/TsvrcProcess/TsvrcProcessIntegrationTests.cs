...
{
    public class TsvrcProcessIntegrationTests : TsvrcProcessTestBase
    {
        [Test]
        ...
        {
            // Two independent TsvrcProcess C# instances stand in for two real clients
            ...
            // would overwrite.
            var clientA = CreateProcess<TsvrcProcessTestSubclass>("ClientA");
            var clientB = CreateProcess<TsvrcProcessTestSubclass>("ClientB");
        ...
        [Test]
        public void TsvrcProcessSubclass_CanUseInheritedPubSub_FromWithinItsOwnHook()
        {
            var process = CreateProcess<EventingProcess>();
            SeedAsOwner(process);
            var listenerDouble = CreateComponent<TsvrcListenerDouble>("Listener");
            process.TsSubscribe(listenerDouble, EventingProcess.DoneEvent, nameof(TsvrcListenerDouble.CallbackA));
